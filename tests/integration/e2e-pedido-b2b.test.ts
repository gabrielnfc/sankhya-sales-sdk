...
// ---- E2E B2B Order Flow ----
describe.skipIf(!has)('E2E Pedido B2B - Fluxo completo', { timeout: 120_000, sequential: true }, () => {
  ...
  it('cria pedido de venda', async () => {
    ...
    try {
      ...
      if (error instanceof ApiError || error instanceof GatewayError) {
        console.warn('E2E Step 4: Criar pedido falhou (config sandbox):', (error as Error).message);
        return;
      }
      throw error;
    }
...
describe('Write safety - no retry on mutations', () => {
  it('HttpClient.requestWithRetry does not retry POST on timeout', async () => {
    ...
    try {
      ...
      await expect(
        httpClient.restPost('/test', { data: 'test' }),
